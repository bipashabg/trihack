...
contract Orderbook {
    ...
    function getTotalCollateral(address borrower) public view returns (uint256 totalCollateral) {
        BorrowPosition[] memory positions = borrowerPositions[borrower];
        ...
        for (uint256 i = 0; i < positions.length; i++) {
            ...
            LendingPool pool = LendingPool(positions[i].pool);
            uint256 collateralShares = pool.collateral(positions[i].poolId, borrower);
